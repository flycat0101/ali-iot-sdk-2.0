...
#else
    #define PRODUCT_KEY             "us86YAUNjC1"
    #define DEVICE_NAME             "ls1021a_perform"
    #define DEVICE_SECRET           "LA7FTp0QCreQjMQ79Lrzs4CA1gkTORHf"
#endif
...
static void _demo_message_arrive(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
	...
	if (result == 0) {
		if (current_stat == 0) {
			...
			sprintf(cmd, "run_thread.sh %s", "on");
			current_stat = 1;
			ret = system(cmd);
		...
		sprintf(cmd, "run_thread.sh %s", "off");
		current_stat = 0;
		ret = system(cmd);
